...
class Rule {
  ...
  freq(dice) { // مجموع تكرار نتائج النرد
  ...
  count(dice, val) { 
    ...
    return dice.filter(d => d === val).length; // مجموع تكرار قيمة معينة
  }
}
...
class TotalOneNumber extends Rule {
  evalRoll = dice => { // مجموع تكرار قيمة معينة ضرب قيمتها مثلا الرقم ثلاثة مكرر مرتين ضرب قيمته وهو 3 * 2 = 6 
...
const fullHouse = new FullHouse({ score: 25, description: '25 points for all house'});

// small/large straights score as 30/40
const smallStraight = new SmallStraight({ score: 30, description: '30 points for all a small straight' });
const largeStraight = new LargeStraight({ score: 40, description: '40 points for all a large straight' });
